...
| `terminal.mjs` | 持久终端：open/exec/task_wait/write/read/tail/search/resize/status/close 端到端（需可达 SSH 主机 + 凭据） | `node tests/terminal.mjs` |
...
覆盖：协议 v5 + `terminal_v1` / `terminal_policy_v2` / `terminal_cursor_v2` / `terminal_task_v1` 能力、effective authorization、terminal_open 摘要信封、terminal_exec→task_wait 任务路径、write→read 输出捕获、absolute cursor 恢复元数据、tail、search、resize、close 后会话移除。
